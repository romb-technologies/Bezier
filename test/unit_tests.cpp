...
class BezierTest : public ::testing::Test {
 public:
  BezierTest()
      : curve_{getCurvePointsAsEigenMatrix()},
        curve_roots_{getRootsCurvePointsAsEigenMatrix()} {}
...
TEST_F(BezierTest, CurveOrderTest) {
  EXPECT_EQ(curve_.order(), 3) << "Curve order differs from expected.";
}
...
TEST_F(BezierTest, CurvePolylineTest) {
  ...
  for (int i = 0; i < polyline.size(); i++) {
    EXPECT_EQ(polyline[i], expected_polyline[i])
        << "Polyline differs from expected";
  }
}

TEST_F(BezierTest, CurveLengthTest) {
  EXPECT_NEAR(curve_.length(), 189.11484365930446, 1e-8)
...
TEST_F(BezierTest, CurveSetControlPointTest) {
  ...
  EXPECT_NO_THROW(curve_.setControlPoint(2, new_control_point))
      << "Curve set control point failed";
  EXPECT_EQ(curve_.controlPoint(2), new_control_point)
      << "Failed to set control point";
}

TEST_F(BezierTest, CurveElevateOrderTest) {
  EXPECT_NO_THROW(curve_.elevateOrder()) << "Curve::elevateOrder failed";
}

TEST_F(BezierTest, CurveLowerOrderTest) {
  EXPECT_NO_THROW(curve_.lowerOrder()) << "Curve::lowerOrder failed";
}
...
TEST_F(BezierTest, CurveValueAtTest) {
  ...
  for (int i = 0; i < t_vals.size(); i++) {
    ...
    EXPECT_EQ(current_point, expected_points[i])
        << "Curve Point at t = " << t_vals[i] << " differs from expected";
  }
}
...
TEST_F(BezierTest, CurveCurvatureAtTest) {
  ...
  for (int i = 0; i < t_vals.size(); i++) {
    ...
    EXPECT_NEAR(current, expected[i], 1e-12)
        << "Curvature values differ from expected";
  }
}
...
TEST_F(BezierTest, CurveCurvatureDerivativeAtTest) {
  ...
  for (int i = 0; i < t_vals.size(); i++) {
    ...
    EXPECT_NEAR(current, expected[i], 1e-12)
        << "Curvature derivative differs from expected";
  }
}
...
TEST_F(BezierTest, CurveTangentAtTest) {
  ...
  for (int i = 0; i < t_vals.size(); i++) {
    ...
    EXPECT_EQ(current, expected[i])
        << "Curve Tangent at t = " << t_vals[i] << " differs from expected";
  }
}
...
TEST_F(BezierTest, CurveNormalAtTest) {
  ...
  for (int i = 0; i < t_vals.size(); i++) {
    ...
    EXPECT_EQ(current, expected[i])
        << "Curve Normal at t = " << t_vals[i] << " differs from expected";
  }
}

TEST_F(BezierTest, CurveDerivativeTest) {
  PointVector expected_control_points{
      {486., -396.}, {-594., 618.}, {396., -378.}};
...
TEST_F(BezierTest, CurveRootsTest) {
  std::vector<double> expected_roots{0.15960764467040239, 0.20703566777685056,
                                     0.81790457456613741};
  std::vector<double> curve_roots = curve_roots_.roots();
  ASSERT_EQ(expected_roots.size(), curve_roots.size())
      << "Roots vector size differs from expected";
  for (int i = 0; i < curve_roots.size(); i++) {
    EXPECT_EQ(expected_roots[i], curve_roots[i])
        << "Curve roots differ from expected";
  }
}

TEST_F(BezierTest, CurveExtremaTest) {
  std::vector<double> expected_extrema{0.69733039467846381,
                                       0.51985862621226775};
  std::vector<double> curve_extrema = curve_roots_.extrema();
  ASSERT_EQ(expected_extrema.size(), curve_extrema.size())
      << "Extrema vector size differs from expected";
  for (int i = 0; i < curve_extrema.size(); i++) {
    EXPECT_EQ(expected_extrema[i], curve_extrema[i])
        << "Curve extrema differs from expected";
  }
}
...
TEST_F(BezierTest, CurveBoundingBoxTest) {
  ...
  EXPECT_EQ(curve_bounding_box.min(), expected_min)
      << "Bounding box minimum differs from expected";
  EXPECT_EQ(curve_bounding_box.max(), expected_max)
      << "Bounding box maximum differs from expected";
}
...
TEST_F(BezierTest, CurveIntersectionsTest) {
  ...
  PointVector expected_intersections{
      {143.28410318501778420, 118.12287028981754133},
      {150.87261159971575353, 120.47985696319850035},
      {179.99996184822515488, 110.00038146265423222},
      {166.28506725972277991, 123.01852075448888968},
      {166.28401126949671607, 123.01855001520399924},
      {128.65721095296441945, 127.43178611658311183}};
  PointVector intersections = curve_.intersections(curve_with_intersections);
  ASSERT_EQ(expected_intersections.size(), intersections.size())
      << "Intersections vector size differs from expected";
  for (int i = 0; i < intersections.size(); i++) {
    EXPECT_EQ(expected_intersections[i], intersections[i])
        << "Curve intersections differs from expected";
  }
}

TEST_F(BezierTest, CurveSplitTest) {
  std::pair<Curve, Curve> split_curves = curve_.splitCurve();
  std::pair<PointVector, PointVector> split_curves_control_points{
      split_curves.first.controlPoints(), split_curves.second.controlPoints()};
  std::pair<PointVector, PointVector> expected_control_points = {
      {{84.00000000000001421, 161.99999999999997158},
  ...
  ASSERT_EQ(curve.size(), expected_curve.size())
      << "First curve control points size differs from expected";
  for (int i = 0; i < curve.size(); i++) {
    EXPECT_EQ(curve[i], expected_curve[i])
        << "First curve control points differ from expected";
  }
  // Check second curve
  curve = split_curves_control_points.second;
  expected_curve = expected_control_points.second;
  ASSERT_EQ(curve.size(), expected_curve.size())
      << "Second curve control points size differs from expected";
  for (int i = 0; i < curve.size(); i++) {
    EXPECT_EQ(curve[i], expected_curve[i])
        << "Second curve control points differ from expected";
  }
}
...
TEST_F(BezierTest, CurveProjectPointTest) {
  ...
  EXPECT_NEAR(t_closest_to_point, 0.03465913829148962, 1e-12)
      << "Calculated `t` differs from expected";
}

TEST_F(BezierTest, CurveDistanceTest) {
  Point point{100, 150};
  double distance_to_point = curve_.distance(point);
  EXPECT_EQ(distance_to_point, 0.68269613683526820)
      << "Calculated distance differs from expected";
}
...
TEST_F(BezierTest, CurveManipulateCurvatureTest) {
  ...
  const PointVector expected_control_points{
      {84.00000000000000000, 162.00000000000000000},
