...
inline double binomial(int n, int k) { return tgamma(n + 1) / (tgamma(k + 1) * tgamma(n - k + 1)); }
...
Curve::Coeffs Curve::elevateOrderCoeffs(int n) const
{
...
Curve::Coeffs Curve::lowerOrderCoeffs(int n) const
{
  if (lower_order_coeffs_.find(n) == lower_order_coeffs_.end())
  {
    lower_order_coeffs_.insert(std::make_pair(n, Coeffs::Zero(n - 1, n)));
    lower_order_coeffs_.at(n) = (elevateOrderCoeffs(n - 1).transpose() * elevateOrderCoeffs(n - 1)).inverse() *
...
{
  N_ = static_cast<int>(points.rows());
...
void Curve::manipulateCurvature(double t, Point point)
{
  ...
  case 4:
    ...
    Point v1 = A - (A - e1) / (1 - t);
    Point v2 = A + (e2 - A) / t;
    control_points_.row(1) = control_points_.row(0) + (v1.transpose() - control_points_.row(0)) / t;
    control_points_.row(2) = control_points_.row(3) - (control_points_.row(3) - v2.transpose()) / (1 - t);
  }
  resetCache();
}
...
BBox Curve::getBBox(bool use_roots) const
{
  if (!(use_roots ? cached_bounding_box_tight_ : cached_bounding_box_relaxed_))
  {
    ...
    {
      for (int k = 0; k < control_points_.rows(); k++)
...
double Curve::projectPointOnCurve(Point point, double step) const
{
  double t = 0;
