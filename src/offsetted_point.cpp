...
inline double LDx(Bezier::Point p, Bezier::Point offset) {
  return  offset.x() * p.x() - offset.y() * p.y();
}

inline double LDy(Bezier::Point p, Bezier::Point offset) {
  return offset.x() * p.y() + offset.y() * p.x(); }

namespace Bezier {

template <>
Bezier::Point getOffsettedPoint<Bezier::Curve>(const Bezier::Curve &curve, double t, Bezier::Point offset)
{
  Bezier::Point p_off;
...
template <>
Bezier::Point getOffsettedPointDerivation_1<Bezier::PolyCurve>(const Bezier::PolyCurve &poly_curve, double t, Bezier::Point offset)
{
  uint idx = static_cast<uint>(t);
  if (idx == poly_curve.getSize()) // for the last point of last curve
    --idx;
...
Bezier::Point getOffsettedPointDerivation_2<Bezier::Curve>(const Bezier::Curve &curve, double t, Bezier::Point offset)
{
  ...
  d2_off.x() = d2.x() + LDx(d3, offset) / d1.norm() - (2 * n2 * LDx(d2, offset) + n1 * LDx(d1, offset)) / std::pow(d1.norm(), 3) +
      3 * n2 * n2 * LDx(d1, offset) / std::pow(d1.norm(), 5);

  d2_off.y() = d2.y() + LDy(d3, offset) / d1.norm() - (2 * n2 * LDy(d2, offset) + n1 * LDy(d1, offset)) / std::pow(d1.norm(), 3) +
...
template <>
Bezier::Point getOffsettedPointDerivation_2<Bezier::PolyCurve>(const Bezier::PolyCurve &poly_curve, double t, Bezier::Point offset)
{
  uint idx = static_cast<uint>(t);
  if (idx == poly_curve.getSize()) // for the last point of last curve
    --idx;
...
template <>
Bezier::Point getOffsettedPointDerivation_3<Bezier::PolyCurve>(const Bezier::PolyCurve &poly_curve, double t, Bezier::Point offset)
{
  uint idx = static_cast<uint>(t);
  if (idx == poly_curve.getSize()) // for the last point of last curve
    --idx;
...
template <>
double getOffsettedPointKappa<Bezier::Curve>(const Bezier::Curve &curve, double t, Bezier::Point offset)
{
...
template <>
double getOffsettedPointKappaDerived<Bezier::PolyCurve>(const Bezier::PolyCurve &poly_curve, double t, Bezier::Point offset)
{
  uint idx = static_cast<uint>(t);
  if (idx == poly_curve.getSize()) // for the last point of last curve
    --idx;
