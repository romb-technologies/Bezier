...
PolyCurve::PolyCurve(std::vector<CurvePtr> &curve_list, const std::vector<PolyCurve::LinkType> &type_list) :
  PolyCurve(curve_list[0], curve_list[1], type_list.empty() ? none : type_list[0])
{
  ...
  for(uint k = 1; k < curve_list.size() - 1; k++)
  {
    ...
    chain_.push_back(Link(curve_list[k], curve_list[k+1], type_list.empty() ? none : type_list[k], point));
  }
}
...
uint PolyCurve::getSize() const
{
  return static_cast<uint>(chain_.size() + 1);
}

int PolyCurve::getIdx(const CurvePtr &curve) const
{
  int idx = -1;
...
{
  if(idx < chain_.size()) return chain_[idx].c[0];
  if(idx == chain_.size() && !chain_.empty()) return chain_.back().c[1];
  return CurvePtr();
}
...
std::vector<Point> PolyCurve::getPointsOfIntersection(const PolyCurve &poly_curve, bool stop_at_first, double epsilon) const
{
  std::vector<Point> points;
